...
\comentario{En la definición antrior es la única vez que aparece el uso
  de medidas para justificar la terminación. Creo que lo mejor es no
  incluir su formalización.}

Tras todo lo detallado, se tiene que al haber introducido la definición de los
números \(m\) y \(p\) anteoriormente descritos en Lean; el estado actual pasa a
ser el siguiente:
\begin{leancode}
n: ℕ
m: ℕ := n.factorial + 1
...
⊢ ∃ (p : ℕ) (H : p ≥ n), prime p
\end{leancode}
\comentario{He modificado las últimas líneas tras la inclusión de la definición.

...
\tactica{linarith}{linarith}. Esta táctica es capaz de probar multitud de
...
\comentario{¿Esta explicaciín sí valdría?}
...
\tactica{cases}{cases} cuando se usa para la regla de eliminación de la
conjunción:
...
En el segundo ejemplo, se puede observar la táctica \tactica{cases}{cases}
...
En este último ejemplo, se presenta la táctica \tactica{cases}{cases} para la
...
\comentario{A ver si le parecen bien estos ejemplos. El primero lo he mantenido
porque si me parecía ilustrativo (si a usted no, busco otro).}
...
explicación de la táctica \tactica{cases}{cases} (en concreto el usado para
ilustrar la regla de eliminación de la disyunción), se puede ver muy bien el
uso de las tácticas \tactica{left / right}{left} y \tactica{left / right}{right}:
\begin{leancode}
example (P Q : Prop): P ∨ Q → Q ∨ P :=
begin
  intro hpq,
...
\end{leancode}
\comentario{He modificado este ejemplo también. Me parece más visual}
...
La táctica \tactica{specialize}{specialize} actúa sobre una hipótesis
determinada y su función consiste en concretar los términos que en la hipótesis
son generales o universales por los que el usuario imponga.
...
\tactica{specialize}{specialize}, se ha particularizado la hipótesis x\(\_\)up
...
La táctica \tactica{push_neg}{push\_neg} puede ser aplicada sobre una hipótesis
o sobre el propio objetivo a demostrar. Básicamente, lo que hace esta táctica es
...
igual que \(x\). Tras la aplicación de la táctica \tactica{push_neg}{push\_neg},
...
\tactica{congr'}{congr'}, la cual sólo ha sido utilizada una vez en el
desarrollo del trabajo. La táctica \tactica{congr'}{congr'} es una táctica muy
parecida a \tactica{congr}{congr} pero menos agresiva que esta última.

Estas tácticas intentan demostrar el objetivo cuando se trata de una igualdad
devolviendo unos determinados subojetivos a probar. La táctica
\tactica{congr'}{congr'} hace uso de un argumento opcional que es el de la
recursividad de las funciones.

Veamos un ejemplo de la táctica \tactica{congr'}{congr'}:
...
Se puede observar que tras aplicar la táctica \tactica{congr'}{congr'} el
...
\comentario{He añadido las tácticas specialize, push neg y congr' que se usan
en el capítulo de elementos y también esta última sección}

...
import tactic
\end{leancode}
\comentario{He simplicado las importaciones de librerías}
...
A continuación, se van a probar dos lemas muy parecidos (puesto que ambos nos
...
\comentario{Las demostraciones de los lemas desigualdad-auxiliar1(2) no se
  corresponden demasiado con la formalización en Lean, pero no veo forma de
  hacer que se parezcan si no es diciendo que se hace a partir de las
  propias hipótesis}
...
\item \mint{lean}|mul_pos : 0 < a → 0 < b → 0 < a * b |
  \indLema{mul\_pos}
\item \mint{lean}|le_of_dvd {a b : ℤ} (bpos : 0 < b) (H : a ∣ b) : a ≤ b  |
  \indLema{le\_of\_dvd}
\item \mint{lean}| not_le {a b : α} : ¬ a ≤ b ↔ b < a |
  \indLema{not\_le} 
\end{itemize}

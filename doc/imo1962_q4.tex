...
Previo a la resolución como tal del problema, se van a introducir una
serie de definiciones que nos servirán para llevar una notación más
compacta:
...
\comentario{Indicar el contexto en el que se trabaja (teoría que se
  importa, espacio de nombres, etc.}
...
\comentario{Añadir lemas y tácticas usados.}
...
\comentario{He simplificado la demostración en Lean. Habría que
  reflejarlo en la siguiente demostración.}
...
\begin{demostracion}
  ...
  esto significa que el numerador es cero. Por tanto, el problema que
  inicialmente planteamos, (\ref{problema2}), quedaría reescrito de la
  siguiente manera:
...
\comentario{Comentar las elevaciones de enteros a reales.}
...
\begin{itemize}
\item \mint{lean}|cos_square : cos x ^ 2 = 1 / 2 + cos (2 * x) / 2 |
  \indLema{cos\_square}
